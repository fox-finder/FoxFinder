...
class DockStore {
  ...
  get apps(): App[] {
    return applicationStore.berthViewApps.slice()
  }
...
const SortableItem = SortableElement(observer(({ app, index }: { app: App, index: number }) => {
  return (
    <DockItem
...
const SortableList = SortableContainer(observer(({ apps }: { apps: App[] }) => {
  return (
    <Background
      tag="ul"
      blur={true}
