...
export class Dock extends React.Component {
  ...
  @action.bound updateDragging(value: boolean) {
    this.isDragging = value
  }

  @computed get viewApps(): Application[] {
    const apps = process.berthViewApps
    if (this.isDragging && this.targetAppId && this.sourceAppId) {

      const newApps = [...apps.slice()]

      const source = newApps.find(app => app.$.id === this.sourceAppId)
      const sourceIndex = newApps.findIndex(app => app === source)
      const target = newApps.find(app => app.$.id === this.targetAppId)
      const targetIndex = newApps.findIndex(app => app === target)

      if (this.targetAppId === this.sourceAppId) {
        return newApps
      }

      newApps.splice(sourceIndex, 1)
      newApps.splice(targetIndex, 0, target as Application)
              ...
              return (
                <React.Fragment key={app.$.id}>
                  <DockItem
                    ...
                    onDragging={() => this.updateDragging(true)}
                    onCancelDragging={() => this.updateDragging(false)}
                  />
                  {/* {renderSeparator && (
                    <DockSeparator />
                  )} */}
